...
- **Published AI QA agent:** [verdict](https://github.com/ArtJack/verdict) — baseline → delta runs, a fact harness (the model judges, the system measures), signed run history, an exit-code CI gate. **73 tagged releases, 1,338 tests**, seven scored eval fixtures plus a 40-instance SWE-bench Verified key, with the **misses published alongside the 8/8s**. Run on strangers' repos, it found a data-loss bug and a **4,595-day-old** permissions bug in boltons, both reported upstream.
...
| **[verdict](https://github.com/ArtJack/verdict)** | A skeptical **AI QA agent with memory**: baseline → delta runs (NEW/REGRESSED), a **fact harness** so every count is measured rather than composed, a **signed run history** its own audit made imitation-proof, flaky quarantine with expiry, and an exit-code **release gate for CI**. Ships **`verdict-qa-mcp`**, a read-only MCP server **listed in the official MCP Server Registry**, runs as **five skills in any coding agent** (`npx skills add ArtJack/verdict`), and has a **local tier** where an 8B model on Ollama carries unattended nights on zero Claude tokens. Seven scored eval fixtures and a 40-instance **SWE-bench Verified** key, **misses published** — including the control arm, where plain Claude Code located the defects just as well at under a seventh of the cost. |
